...
fn parse(input: impl Read) -> Result<(Vec<ListItem>, Vec<((u8, u8), u8)>, u8), DynError> {
    ...
    let initial = lines
        ...
        .map(|(index, value)| {
            ...
            ListItem{ value, next: index + 1 }
        })
        .collect();

    let rules = lines.skip(1).map(|line| { 
        let line = line?;
...
fn try_apply_rules(values: &mut Vec<ListItem>, rules: impl Iterator<Item=Result<((u8, u8), u8), DynError>>) -> Result<(), DynError>  {
...
fn apply_rules(value: &mut Vec<ListItem>, rules: &HashMap<(u8, u8), u8>) {
    ...
    for _ in 0..value.len() - 1 {
        ...
        if let Some(insert) = rules.get(&(current.1.value, next.value)) {
            value.push(ListItem{ value: *insert, next: current.1.next });
...
fn apply_n_rules_fast(value: Vec<ListItem>, rules: Vec<((u8, u8), u8)>, n: usize, unique: u8) -> MinMaxResult<u64> {
    ...
    actual.chunks(unique).map(|x| x.iter().sum::<u64>()).minmax()
}
...
mod tests {
    ...
    #[test]
    fn part1() {
        let (mut init, rules, _) = parse(std::fs::File::open("puzzleData/day14.txt").unwrap()).unwrap();
        let rules_map : HashMap<_, _> = rules.into_iter().collect();
        for _ in 0..10 {
            apply_rules(&mut init, &rules_map);
        }
    ...
    #[test]
    fn part2() {
        let (init, rules, unique) = parse(std::fs::File::open("puzzleData/day14.txt").unwrap()).unwrap();
